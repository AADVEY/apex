...
<header>
  ...
  <div class="hdr-center">
    ...
    <div class="hdr-pill" id="clock">--:--</div>
  </div>
  <div class="hdr-right">
    <div>MARKET INTELLIGENCE TERMINAL</div>
    <div style="color:var(--muted)">DATA · YFINANCE + NEWSAPI</div>
  </div>
</header>
...
<div class="wrap">
  ...
  <div id="tab-overview">
    ...
    <div class="panels">
      <div class="panel">
        <div class="ptitle">Price · EMA20 · EMA50 · Monte Carlo Forecast</div>
...
function tick() {
  ...
  const isOpen = isWeekday && ((h === 13 && m >= 30) || (h > 13 && h < 20));
...
function drawMainChart(d) {
  ...
  const labels = d.history_dates.concat(d.forecast_dates);
  ...
  charts['main'] = new Chart(ctx, {
    ...
    options:{
      ...
      plugins:{
        ...
        annotation:{annotations:{fl:{type:'line',xMin:hLen-1,xMax:hLen-1,borderColor:'rgba(240,180,41,.2)',borderWidth:1,borderDash:[4,4],
      ...
      scales:{
        x:{ticks:{color:'#1c2235',font:{family:"'DM Mono',monospace",size:8},maxTicksLimit:8,maxRotation:0},grid:{color:'rgba(28,34,53,.5)'}},
        y:{position:'right',ticks:{color:'#1c2235',font:{family:"'DM Mono',monospace",size:8},callback:v=>'$'+v.toFixed(0)},grid:{color:'rgba(28,34,53,.5)'}}
      }
    }
  });
}
